...
def main():
    ...
    combined = {"mass_gev": float(model.GetParameter(1)), "mass_error_gev": float(model.GetParError(1)), "sigma_gev": abs(float(model.GetParameter(2))), "truth_mass_gev": MASS_KS, "runs": len(rows), "entries": int(merged.GetEntries())}
    ...
    lines = ["runs={}".format(len(rows)), "entries={}".format(int(merged.GetEntries())), "combined_mass_gev={:.9f}".format(combined["mass_gev"]), "combined_mass_error_gev={:.9f}".format(combined["mass_error_gev"]), "combined_sigma_gev={:.9f}".format(combined["sigma_gev"]), "generation_mass_gev={:.9f}".format(MASS_KS)]
